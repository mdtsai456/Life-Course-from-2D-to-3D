...
**Goal:** 以 Docker Compose 加入 **Nginx** 服務，對外只開 **80**（可選環境變數覆寫），提供前端 production 靜態檔並將 **`/api/`** 反代至內網 **backend**；**backend** 與 **triposr-api** 不對主機發布埠；**CORS** 預設涵蓋本機同源入口。
...
| `.dockerignore`（repo 根目錄） | 新建 | 縮小 build context（`node_modules`、`dist`、`.git` 等） |
| `compose.yaml` | 修改 | 新增 `nginx`、backend 健康檢查、拿掉 backend 對外埠、CORS 預設、nginx 依賴 |
| `README.md` | 修改 | 「一鍵起服務」：前置條件（GPU、TripoSR 目錄）、`CORS_ALLOWED_ORIGINS`／`HTTP_PORT`、驗證用 curl |
...
### Task 2: 新增 repo 根目錄 `.dockerignore`

**Files:**

- Create: `.dockerignore`

**完整檔案內容（僅此四行，以降低 build context；若日後需要排除更多路徑再擴充）：**
...
- [ ] **Step 1:** 在 repo 根目錄建立 `.dockerignore`，內容與上列完全一致。

- [ ] **Step 2:** Commit

```bash
git add .dockerignore
git commit -m "chore: add root .dockerignore for faster compose builds"
```
...
```yaml
      CORS_ALLOWED_ORIGINS: ${CORS_ALLOWED_ORIGINS:-http://127.0.0.1,http://localhost}
```
...
| 變數 | 預設 | 說明 |
|------|------|------|
| `HTTP_PORT` | `80` | 對外 HTTP 埠（對應 nginx 服務 `ports`） |
| `CORS_ALLOWED_ORIGINS` | `http://127.0.0.1,http://localhost` | 逗號分隔。若以區網 IP 從其他裝置開網頁，請加上 `http://<你的區網IP>`（勿在不明環境使用 `*`）。 |

**段落 C — 標題 `**驗收（主機上，Compose 已 up）：**` 後接第一段 curl：**

```bash
curl -sS -o /dev/null -w "%{http_code}" http://127.0.0.1:${HTTP_PORT:-80}/
```
...
```bash
curl -sS -o /dev/null -w "%{http_code}" http://127.0.0.1:${HTTP_PORT:-80}/health
```

接一行說明：預期輸出 `200`（JSON 內 `triposr_ok` 是否為 true 依 GPU／服務狀態而定，但 HTTP 應為 200）。

**段落 E — 結尾一句：** 瀏覽器開 `http://127.0.0.1`（或 `http://localhost`）應載入前端；上傳圖片跑完整流程須 GPU 與 triposr-api healthy。
...
| Spec 段落 | 對應任務 |
|-----------|----------|
| 單一對外埠 80、可覆寫 | Task 5 `HTTP_PORT`、Task 6 文件 |
